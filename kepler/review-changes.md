...
All of that happens in the task's **worktree** — the Git working copy the agent made its changes in. The task view rail groups a task's worktrees under **Changes**, which is what you go there for; open one from that group and it takes a column of its own. See [the task view](/kepler/task-view).
...
Its header carries the branch, the repository, how far the branch is ahead of or behind its remote as **↑n** and **↓n**, and three controls, ordered by how far each takes you from the pane: the worktree's configured commands, **Open in** for handing the checkout to another app, and **Terminal** for a plain shell. A command you run opens its terminal as a tab in this same column.
...
When the branch has diverged from its upstream, the graph draws both lanes so you can see the split. Branch chips are tinted to their lane, with a monitor glyph for a local branch and a cloud glyph for a remote one; tags render as **#name** pills.
...
Discarding always asks first — **Discard changes?**, naming the file, or the number of files, and saying that it cannot be undone. Staging never asks; it is reversible.

Right-click a file row for **Copy path**, **Open containing folder** where this window can reach the file manager, and **Open file on remote** on rows that belong to a commit.
...
**Commit** stays disabled until the message has content. On a repository configured to sign commits, a signing failure keeps your draft message in the box and explains itself in a banner rather than discarding what you typed.
...
**Kepler has no "create pull request" button.** The pull request gets opened by the agent, by a command you run in the worktree's terminal, or on your host's website. Kepler's part starts once it exists: it tracks the pull request as a task resource. Three paths attach one.
...
| **The agent attaches it** | Agents can attach a link to the task themselves through Kepler's workspace tools. Kepler works out from the URL whether it is a pull request, an issue, or a plain link, and fetches its title and status where it can; the link is attached even when that lookup fails. The same tools detach one, including a link you attached yourself |
...
An agent is asked to attach only the pull request the task is actually about, not every URL it reads while investigating, and anything it attaches shows up in the task straight away.
...
Two shipped features that hand agents Git tooling they do not otherwise have. Both are off until you turn them on, and both need a **paid GitKraken subscription**. Enable them in **Settings → Agents → Features**.
...
|---|---|
| **AI Sync** | Runs the rebase or merge, resolving conflicts as it goes, then reports the result for you to accept or roll back. Every run is backed up first. It also drops commits whose changes are already upstream, which is what makes it useful after a squash-merge |
