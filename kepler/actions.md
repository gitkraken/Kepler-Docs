...
Actions replace the generic *Start* button and the prompts that used to be hardcoded in Kepler. You pick the wording, you pick the agent, and you pick which Action is the one-click default for each kind of work.

<!-- TODO(screenshot): the Action split button on a Todo row, chevron open, showing all four Actions. -->
...
An Action fires immediately unless the firing is genuinely ambiguous. Only one thing can be ambiguous: **where to run it**.
...
<!-- TODO(screenshot): the Run in popover — New session / Current session toggle with worktree rows beneath. -->

***

## Set your preferred Actions

Open **Settings → Actions → Preferred actions**. Each row sets the Action that the left half of the button runs for that kind of item.
...
Any slot can be set to **None**, in which case that surface shows the chevron alone.

<!-- TODO(screenshot): Settings → Actions showing Preferred actions and the Actions list. -->

***

## Edit an Action

In **Settings → Actions**, click the **Edit** (pencil) icon on any Action. Built-in Actions appear under **Built in**; your own appear under **Custom**.
...
Click **New action**. Custom Actions work exactly like the built-in ones, and they're the point of the feature: if you'd do something more than once — backport a fix, write the migration, audit a dependency bump — it should be a button rather than a prompt you retype.

### Restore defaults

**Restore default** on a row, or **Restore all defaults** at the bottom of the list.

Kepler stores **only what you changed**, not a copy of the whole Action. Two consequences:
...
Deleting a custom Action that a preferred-Action slot points at leaves that slot showing the chevron alone, the same as setting it to **None**.

***

## The agent an Action runs on
